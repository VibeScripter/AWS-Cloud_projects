...
- We need to go to directory that our file exists. Before we run our VBoxes using `vagrant`, we need to install below plugin.
```sh
vagrant plugin install vagrant-hostmanager
```

- After plugins install, we can run the below commands to setup our VMs.
```sh
vagrant up
```
...
- NOTE: Set db root password, we will be using `admin123` as password. Below is what it will look like
```
...
```
git clone https://github.com/VibeScripter/vprofile/tree/main
